...
def parse_args():
    ...
    ebnf_opts.add_argument('--color', '-c',
                           help='use color in traces (requires the colorama library)',
                           action='store_true'
                           )
    ebnf_opts.add_argument('--trace', '-t',
...
def genmodel(name=None, grammar=None, **kwargs):
    return GrakoGrammarGenerator(name, **kwargs).parse(grammar, **kwargs)
